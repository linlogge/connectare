        ...
        buf.push_str(
            quote! {
                ...
                pub struct #client_name {
                    client: connectare::client::RpcClient,
                }

                #[allow(dead_code)]
                impl #client_name {
                    pub fn new(client: connectare::client::RpcClient) -> Self {
                        Self { client }
                    }

                    pub fn from_config(config: connectare::client::RpcClientConfig) -> Self {
                        Self {
                            client: connectare::client::RpcClient::new(config),
                        }
                    }
        ...
        if method.server_streaming {
            quote! {
                pub fn #method_name<T, H, S>(
                    handler: H
                ) -> impl FnOnce(axum::Router<S>) -> connectare::server::router::RpcRouter<S>
                where
                    H: connectare::server::handler::RpcHandlerStream<#input_type, #output_type, T, S>,
                    T: 'static,
            ...
            quote! {
                ...
                where
                    H: connectare::server::handler::RpcHandlerUnary<#input_type, #output_type, T, S>,
                    T: 'static,
                    S: Clone + Send + Sync + 'static,
                {
                    move |router: axum::Router<S>| {
        ...
        if method.server_streaming {
            quote! {
                pub async fn #method_name(
                    &self,
                    request: #input_type,
                ) -> Result<connectare::client::RpcStream<#output_type>, connectare::server::error::RpcError> {
            ...
            quote! {
                pub async fn #method_name(
                    &self,
                    request: #input_type,
                ) -> Result<#output_type, connectare::server::error::RpcError> {
